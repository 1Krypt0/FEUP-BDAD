...
estado TEXT NOT NULL REFERENCES estado(designacao),
rua TEXT NOT NULL REFERENCES localizacao(rua),
nr_porta INTEGER NOT NULL REFERENCES localizacao(nr_porta),
cod_postal TEXT NOT NULL REFERENCES localizacao(cod_postal)
);
...
tipo TEXT NOT NULL REFERENCES tipo_de_veiculo(designacao),
...
nif INTEGER NOT NULL PRIMARY KEY CONSTRAINT nif_valido CHECK(nif > 0) ON UPDATE CASCADE,
nome TEXT NOT NULL,
contacto INTEGER NOT NULL CONSTRAINT contacto_valido CHECK((contacto >= 900000000 AND contacto <= 999999999) OR (contacto >= 200000000 and contacto <= 299999999)),
...
nif INTEGER NOT NULL PRIMARY KEY REFERENCES empregado(nif),
estacao_trabalho TEXT NOT NULL UNIQUE,
especialidade TEXT NOT NULL REFERENCES especialidade(designacao)
);
...
codigo_material INTEGER NOT NULL REFERENCES material(codigo),
nr_obra INTEGER NOT NULL REFERENCES obra(nr_obra),
PRIMARY KEY(codigo_material,nr_obra)
);

DROP TABLE IF EXISTS veiculo_alocado;
CREATE TABLE veiculo_alocado(
matricula INTEGER NOT NULL REFERENCES veiculo(matricula),
nr_obra INTEGER NOT NULL REFERENCES obra(nr_obra),
...
CREATE TABLE conduz(
nif TEXT NOT NULL REFERENCES maquinista(nif),
tipo_veiculo INTEGER NOT NULL REFERENCES veiculo(designacao),
PRIMARY KEY(categoria_maquinista,tipo_veiculo)
);

DROP TABLE IF EXISTS cliente_obra_projeto;
CREATE TABLE cliente_obra_projeto(
nif_cliente INTEGER NOT NULL REFERENCES cliente(nif),
nr_obra INTEGER NOT NULL REFERENCES obra(nr_obra),
projeto INTEGER NOT NULL PRIMARY KEY REFERENCES projeto(nr_projeto)
);
